...
const OMAR_AI_DESKTOP_BASE_URL = process.env.OMAR_AI_DESKTOP_BASE_URL as string;
...
async function moveMouse(input: { coordinates: Coordinates }): Promise<void> {
  ...
  try {
    await fetch(`${OMAR_AI_DESKTOP_BASE_URL}/computer-use`, {
      method: 'POST',
...
async function cursorPosition(): Promise<Coordinates> {
  ...
  try {
    const response = await fetch(`${OMAR_AI_DESKTOP_BASE_URL}/computer-use`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
