...
export function ChatInput({
  ...
  onFileUpload,
  minLines = 1,
  placeholder = "Give OMAR AI a task to work on...",
}: ChatInputProps) {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="relative">
        <textarea
          ...
          className={cn(
            "placeholder:text-omar-ai-bronze-light-10 w-full rounded-lg py-2 pr-16 pl-3 placeholder:text-[13px]",
            "file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-omar-ai-bronze-light-7 flex min-w-0 border bg-transparent text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
        ...
        <div className={`absolute right-2 ${buttonPositionClass} flex items-center gap-1`}>
          ...
          {isLoading ? (
            <div className="border-omar-ai-bronze-light-7 border-t-primary h-5 w-5 animate-spin rounded-full border-2" />
          ) : (
            <Button
              type="submit"
              variant="ghost"
              size="icon"
              className="bg-omar-ai-bronze-dark-7 hover:bg-omar-ai-bronze-dark-6 h-6 w-6 cursor-pointer rounded-sm"
              disabled={isLoading}
            >
              <HugeiconsIcon
