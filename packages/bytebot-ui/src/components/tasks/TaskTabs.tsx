...
const TAB_CONFIGS: Record<TabKey, TabConfig> = {
  ALL: {
    label: "All",
    icon: ListViewIcon,
    color: "text-omar-ai-bronze-light-10",
...
export const TaskTabs: React.FC<TaskTabsProps> = ({
  ...
  return (
    <div className="border-omar-ai-bronze-light-7 mb-6 border-b">
          ...
          return (
            <button
              ...
              className={`flex cursor-pointer items-center space-x-2 border-b-2 px-4 py-3 whitespace-nowrap transition-colors ${
                isActive
                  ? "border-omar-ai-bronze-dark-7 text-omar-ai-bronze-dark-7"
                  : "text-omar-ai-bronze-light-10 hover:text-omar-ai-bronze-dark-7 border-transparent"
              }`}
            >
              <HugeiconsIcon
                icon={config.icon}
                className={`h-4 w-4 ${isActive ? "text-omar-ai-bronze-dark-7" : config.color}`}
              />
              ...
              {count > 0 && (
                <span
                  className={`rounded-full px-2 py-0.5 text-xs ${
                    isActive
                      ? "bg-omar-ai-bronze-dark-7 text-white"
                      : "bg-omar-ai-bronze-light-7 text-omar-ai-bronze-light-11"
                  }`}
                >
                  {count}
                </span>
